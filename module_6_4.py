...
class Krug(Figure):
    ...
    def __init__(self, color, *sides):
        ...
        self.__radius = sides[0] / (2 * math.pi) if sides else 1 / (2 * math.pi)

    def get_square(self):
        return math.pi * (self.__radius ** 2)

class Triangul(Figure):
    sides_count = 3
...
class Cube(Figure):
    ...
    def __init__(self, color, *sides):
        super().__init__(color, *sides)
        self.set_sides(*([sides[0]] * self.sides_count) if len(sides) == 1 else [1] * self.sides_count)
...
circle1 = Krug((200, 200, 100), 10) # (Цвет, стороны)
